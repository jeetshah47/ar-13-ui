...
import { getUserPermissionsAction, updateUserAction } from "../../../store/features/user/userAction";
import type { Permission, UserRole } from "../../../store/types/RBAC";
import type { UpdateUserRequest } from "../../../store/types/User/UpdateUserRequest";
...
const EmployeePermissionsSection = ({ userId }: EmployeePermissionsSectionProps) => {
  const { isAdmin } = usePermissions();
  const theme = useTheme();
  const dispatch = useAppDispatch();
  const { permissions, permissionsLoading, permissionsError } = useAppSelector(
  ...
  const fetchPermissions = () => {
    if (userId) {
      dispatch(getUserPermissionsAction(userId));
    }
  };
  ...
  const handleUpdatePermissions = async (_permissions: Permission[], role?: UserRole) => {
    ...
    try {
      await dispatch(updateUserAction(updateData));
  ...
  // Error state
  if (error && userPermissions.length === 0) {
    return (
      <Box sx={{ padding: "24px" }}>
        <Card
          sx={{
